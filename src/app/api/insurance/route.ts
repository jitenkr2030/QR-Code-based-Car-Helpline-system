...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (type) {
      ...
      case 'policies':
        return await getInsurancePolicies(userId || undefined, vehicleId || undefined)
      case 'claims':
        return await getInsuranceClaims(userId || undefined, policyId || undefined, status || undefined)
      case 'policy':
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { type, userId, vehicleId, providerId, policyId, policyData, claimData } = body
...
async function createInsuranceClaim(policyId: string, claimData: any) {
  ...
  const claim = await db.insuranceClaim.create({
    data: {
      ...
      documents: claimData.documents ? JSON.stringify(claimData.documents) : null,
