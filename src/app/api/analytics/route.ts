...
async function getUserAnalytics(period?: string, startDate?: string, endDate?: string) {
  ...
  return {
    ...
    avgQROrdersPerUser: activeUsers.reduce((sum, user) => sum + user._count.qrcodeOrders, 0) / activeUsers.length || 0,
    ...
    userActivity: {
      ...
      totalQROrders: activeUsers.reduce((sum, user) => sum + user._count.qrcodeOrders, 0),
...
async function getServiceAnalytics(period?: string, startDate?: string, endDate?: string) {
  ...
  const avgServiceTime = completedBookings.reduce((sum, booking) => {
    if (booking.updatedAt && booking.assignedAt) {
      const serviceTime = new Date(booking.updatedAt).getTime() - new Date(booking.assignedAt).getTime()
