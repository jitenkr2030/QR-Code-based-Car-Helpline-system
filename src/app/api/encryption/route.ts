  ...
  try {
    ...
    switch (action) {
      ...
      case 'decrypt-file':
        ...
        let fileData
        try {
          fileData = typeof data === 'string' ? JSON.parse(data) : data
        } catch {
          return NextResponse.json(
            { error: 'Bad Request', message: 'Invalid data format' },
            { status: 400 }
        ...
        if (!fileData || !fileData.encryptedData) {
          return NextResponse.json(
        ...
        const encryptedFileData = {
          encryptedData: Buffer.from(fileData.encryptedData, 'base64'),
          iv: fileData.iv,
          tag: fileData.tag,
          keyId: keyId || 'default'
        }
      ...
      case 'verify-password':
        ...
        let passwordData
        try {
          passwordData = typeof data === 'string' ? JSON.parse(data) : data
        } catch {
          return NextResponse.json(
            { error: 'Bad Request', message: 'Invalid data format' },
            { status: 400 }
        ...
        if (!passwordData.password || !passwordData.hash || !passwordData.salt) {
          return NextResponse.json(
        ...
        const isValid = verifyPassword(passwordData.password, passwordData.hash, passwordData.salt)
      ...
      case 'verify-hmac':
        ...
        let hmacData
        try {
          hmacData = typeof data === 'string' ? JSON.parse(data) : data
        } catch {
          return NextResponse.json(
            { error: 'Bad Request', message: 'Invalid data format' },
            { status: 400 }
        ...
        if (!hmacData.message || !hmacData.key || !hmacData.hmac) {
          return NextResponse.json(
        ...
        const isValidHMAC = verifyHMAC(hmacData.message, hmacData.key, hmacData.hmac)
      ...
      case 'check-strength':
        ...
        let strengthData
        try {
          strengthData = typeof data === 'string' ? JSON.parse(data) : data
        } catch {
          return NextResponse.json(
            { error: 'Bad Request', message: 'Invalid data format' },
            { status: 400 }
        ...
        const strength = checkPasswordStrength(strengthData.password)
